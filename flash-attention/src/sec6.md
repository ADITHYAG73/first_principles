...
Plug in the A100-class numbers: $M = 100{,}000$, $d = 64$ → $B \approx 391$ rows per block. Do the division yourself — its your first rent calculation.

**and now the audit that breaks the square plan.** we forgot a fifth tenant: the minted score tile itself, $B_r \times B_c$. At $B = 391$: $391 \times 391 \approx 153{,}000$ numbers — **bigger than the entire bench**, before the four slabs even sit down. Bankrupt.
...
$$B_c = \left\lceil \frac{M}{4d} \right\rceil \approx 391 \qquad\qquad B_r = \min\!\left(\left\lceil \frac{M}{4d} \right\rceil,\; d\right) = 64$$

Only 64 askers shop at a time, while the shelf of keys stays long at 391. Why cap the askers and not the shelf? Two reasons, and the second is the punchline. First: the shelf *wants* to be long — a long shelf means fewer sections, fewer rounds, and (as the schedule below shows) fewer re-hauls. Second: at $B_r = d$, look at the tile's rent —

$$B_r \times B_c = 64 \times 391 = 391 \times 64 = B_c \times d$$
...
Tally the toy world's bill — the comic at the end of this section lays this schedule out page by page, with the timetable to count from, if you want to do it yourself — and the O-traffic dominates: the wet paint's round-trips are the single biggest line item.
...
Count again — same comic, second timetable — and the bill drops. Because now **the wet paint never travels**: a resident asker's blend stays on the bench through every parade, dries, and ships out exactly once, finished. The O round-trips vanish. The verdict, one sentence: **the light traveler should commute.** the shelf carries two trucks a visit; the askers carry three; FA-1 sat the light party and commuted the heavy one.
